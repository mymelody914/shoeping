        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                if(voview != null){
	                ...
	                $("#aUpdateModal").find(".modal-content").html(`
	                	<form action="/answer/update.do" method="post">
	                		<input type="hidden" name="qno" value="${ '${answerData.qno}' }">
	                		<input type="hidden" name="ano" value="${ '${answerData.ano}' }">
	                		<input type="hidden" name="itemNo" value="${ '${answerData.itemNo}' }">
	            	      <div class="modal-header">
	            	        <button type="button" class="close" data-dismiss="modal">&times;</button>
	            	        <h4 class="modal-title">답변 수정 폼</h4>
	            	      </div>
	            	      <div class="modal-body">
	            	      	<p style="word-break:keep-all; word-wrap:break-word;  margin-bottom: 10px;">
	            	      		<span style="font-weight: bolder;">문의명 : </span>${ "${qnaData.title}" }
	            	      	</p>
	          	      		<p style="word-break:keep-all; word-wrap:break-word;  margin-bottom: 10px;">
	          	      			<span style="font-weight: bolder;">문의 내용 : </span>${ "${qnaData.content}" }
	          	      		</p>
	            	        <textarea rows="3" name="content" class="form-control" id="modalContent" 
		            	        title="내용을 입력해 주세요." style="resize: none;" required>${ "${answerData.content}" }</textarea>
	            	      </div>
	            	      <div class="modal-footer">
	            	        <button class="btn btn-default">수정</button>
	            	        <button type="button" class="btn btn-default" data-dismiss="modal">취소</button>
	                ...
	                `);
	                $("#aWriteModal").find(".modal-content").html(`
	                	<form action="/answer/write.do" method="post">
	                		<input type="hidden" name="qno" value="${ '${qnaData.qno}' }">
	                		<input type="hidden" name="itemNo" value="${ '${qnaData.itemNo}' }">
	                		<input type="hidden" name="title" value="${ '${qnaData.title}' }">
	            	      <div class="modal-header">
	            	        <button type="button" class="close" data-dismiss="modal">&times;</button>
	            	      ...
	            	      <div class="modal-body">
	            	        ...
	            	        <textarea rows="3" name="content" class="form-control" id="modalContent"
	            	        	 title="내용을 입력해 주세요." style="resize: none;" required></textarea>
	            	      </div>
	            	      <div class="modal-footer">
...
<h1>문의</h1>
	...
	<c:if test="${!empty login && login.gradeNo < 9}">
		<button type="button" class="btn btn-default qWriteBtn" data-toggle="modal" data-target="#qWriteModal">등록</button>
	</c:if>
	<c:if test="${pageObject.endRow == 1}">
		<div>
			<pageNav:pageNav listURI="list.do" pageObject="${pageObject}"></pageNav:pageNav>
		</div>
	</c:if>
<!-- qWriteModal -->
<div id="qWriteModal" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
		<form action="/question/write.do" method="post">
			...
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal">&times;</button>
				<h4 class="modal-title">질문 등록 폼</h4>
			</div>
			<div class="modal-body">
				<div style="padding-bottom: 10px;">
					<label class="radio-inline"><input type="radio" name="pw"
						value="0" checked>일반글</label> <label class="radio-inline"><input
						type="radio" name="pw" value="1">비밀글</label>
				</div>
				<div class="form-group">
					<label for="title">제목</label> <input class="form-control"
						name="title" id="title" required pattern="^.{3,100}$" 
						title="3자 이상 100자 이내로 작성하셔야 합니다." maxlength="100">
				</div>
				<div class="form-group">
					<label for="content">내용</label>
					<textarea rows="5" class="form-control" name="content"
						id="content" required title="내용을 입력해 주세요." style="resize: none;"></textarea>
				</div>
			</div>
			<div class="modal-footer">
				<button class="btn btn-default">등록</button>
...
<!-- aUpdateModal -->
<div id="aUpdateModal" class="modal fade" role="dialog">
  <div class="modal-dialog">
